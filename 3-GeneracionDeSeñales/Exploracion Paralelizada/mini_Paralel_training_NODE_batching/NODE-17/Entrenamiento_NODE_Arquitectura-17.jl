...
##############################################################################################
# Función que idientifca las señales que se van a utilizar para el entrenamiento de la NODE con lcm y sigma
...
function Get_Signals_Test_Parameters(rango,lcms,sigmas)
    ...
    column_sigmass_test = column_sigmas[rango]
...
# Definimos el tiempo de prediccion y de entrenamiento
tforecast = t[23:end]
t = t[1:22]

# Tomamos el rango de prueba y el rango de validación
...
Signals_test_derivadas_valid = Signals_test_derivadas[23:end,:]

train_loader = Flux.Data.DataLoader((Signals_test_train, t), batchsize = batch_size)

#[2, 32, 64, 64, 32, 16, 1]

nn = Chain(Dense(2,32, swish),
            Dense(32,64, swish),
            Dense(64,64, swish),
            Dense(64,32, swish),
            Dense(32,16, swish),
...
θ = CSV.read("C:\\Users\\Propietario\\Desktop\\ib\\Tesis_V1\\Proyecto_Tesis\\3-GeneracionDeSeñales\\Exploracion Paralelizada\\mini_Paralel_training_NODE_batching\\Parameters\\17_Parameters.csv", DataFrame)
...
callback = function ()
    ...
    if iter % (length(train_loader)) == 0
        ...
        forecast_loss = loss_node(Signals_test_valid, tforecast)
...
constant = ones(length(linspace))*t[22]

size(Signals_test)[1]

plot_signals = plot(vcat(t_short,t_long), Signals_test[1,:], label = "Señal 1", xlabel = L"t", ylabel = L"S(t)", title = "Predicción de señales", lw = 2, color = :blue, markershape = :circle, tickfontsize=12, labelfontsize=15, legendfontsize=5, framestyle =:box, gridlinewidth=1, xminorticks=10, yminorticks=10)
plot!(t, Predict_Singals(U0[1], Signals_test_derivadas_train[:,1], t), color = :red, label = "Entrenamiento", markershape = :star6, ls = :dash, lw = 2)
plot!(tforecast, Predict_Singals(U0[1], Signals_test_derivadas_train[:,1], tforecast), color = :red, label = "Predicción", markershape = :star6, ls = :dash, lw = 2)
plot!(constant, linspace, label = false, lw = 2, ls = :dash, color = :gray)


plot!(t, Predict_Singals(U0[end], Signals_test_derivadas_train[:,end], t), color = :red, label = "Entrenamiento", markershape = :star6, ls = :dash, lw = 2)
plot!(tforecast, Predict_Singals(U0[end], Signals_test_derivadas_train[:,end], tforecast), color = :red, label = "Entrenamiento", markershape = :star6, ls = :dash, lw = 2)

for i in 2:size(Signals_test)[1]
    plot(vcat(t_short,t_long), Signals_test[i,:], label = "Señal $i", xlabel = L"t", ylabel = L"S(t)", title = "Predicción de señales", lw = 2, color = :blue, markershape = :circle, tickfontsize=12, labelfontsize=15, legendfontsize=5, framestyle =:box, gridlinewidth=1, xminorticks=10, yminorticks=10)
    plot!(t, Predict_Singals(U0[i], Signals_test_derivadas_train[:,i], t), color = :red, label = "Entrenamiento", markershape = :star6, ls = :dash, lw = 2)
    display(plot!(tforecast, Predict_Singals(U0[i], Signals_test_derivadas_train[:,i], tforecast), color = :red, label = "Entrenamiento", markershape = :star6, ls = :dash, lw = 2))
end

plot_signals

for i in 1:size(Signals_test)[1]
    plot(t, Signals_test_train[i,:], label = "Señal $i", xlabel = L"t", ylabel = L"S(t)", title = "Predicción de señales", lw = 2, color = colors[i], markershape = :circle, tickfontsize=12, labelfontsize=15, legendfontsize=15, framestyle =:box, gridlinewidth=1, xminorticks=10, yminorticks=10)
    display(plot!(t, Predict_Singals(U0[i], Signals_test_derivadas_train[:,i], t), color = colors_prediction[i], label = "Predicción $i", markershape = :star6, ls = :dash, lw = 2))
    display(plot!(tforecast, Predict_Singals(U0[i], Signals_test_derivadas_train[:,i], tforecast), color = colors_prediction[i], label = "Predicción $i", markershape = :star6, ls = :dash, lw = 2))
    display(plot!(constant, linspace, label = false, lw = 2, ls = :dash, color = :gray))
...
for i in 1:length(colors)
    if i == 1        
        actual_plot = plot(vcat(t,tforecast), Signals_test[i,:], label = "Señal Real", xlabel = L"$t$ (s)", ylabel = L"S(t)", title = L"Predicción de señal $l_{cm} = $" * " $(column_lcm_test[i]), " * L"sigma = " * "$(column_sigmass_test[i])", lw = 2, color = colors[i], tickfontsize=12, labelfontsize=11, legendfontsize=1, framestyle =:box, gridlinewidth=1, xminorticks=10, yminorticks=10, legend_position = :bottomleft)
    ...
    # display(plot(t, Signals_test[i,:], label = "Señal $i", xlabel = L"t", ylabel = L"S(t)", title = "Predicción de señal lcm = $(column_lcm_test[i]), sigma = $(column_sigmas_test[i])", lw = 2, color = colors[i], markershape = :circle, tickfontsize=12, labelfontsize=15, legendfontsize=15, framestyle =:box, gridlinewidth=1, xminorticks=10, yminorticks=10))
...
##############################################################################################
# Función que idientifca las señales que se van a utilizar para el entrenamiento de la NODE con lcm y sigma
# function Get_Signals_Test_Valid_Parameters(numSignals,lcms,sigmas)
#     dim1 = dimlcm = length(lcms)
#     dim2 = dimsigma = length(sigmas)
...
#     column_sigmas_test = column_sigmas[10:Int(length(column_sigmas)/numSignals):end]
    
#     return column_lcm_test, column_sigmas_test
# end

# # Función que devuelve señales de validacion, sus derivadas y los parámetros con los que se generaron
# function Get_Signals_Data_Valid(path_read, numSignals, lcms, sigmas, muestreo_corto, muestreo_largo)
#     Signals_test = Get_Signals_Test_Valid(numSignals, path_read, muestreo_corto, muestreo_largo)
#     column_lcm_test, column_sigmas_test = Get_Signals_Test_Valid_Parameters(numSignals,lcms,sigmas)
#     Signals_test_derivadas = zeros(size(Signals_test))
#     for i in 1:size(Signals_test)[1]
#         Signals_test_derivadas[i,:] = derivate_signals(t,Signals_test[i,:])
#     end
...
#         Signals_test_derivadas[:,i] = Signals_test_derivadas[:,i] ./ maximum(abs.(Signals_test_derivadas[:,i]))
#     end
#     return Signals_test, Signals_test_derivadas, column_lcm_test, column_sigmas_test
# end


# Signals_valid, Signals_valid_derivadas, column_lcm_valid, column_sigmas_valid = Get_Signals_Data_Valid(path_read, numSignals, lcms, sigmas, muestreo_corto, muestreo_largo)

# Plots_Valid = []

# # Create a 2x2 plot
# for i in 1:length(colors)
#     actual_plot = plot(t, Signals_valid[i,:], label = "Señal validación $i", xlabel = L"t", ylabel = L"S(t)", title = L"Predicción de señal $l_{cm} = $" * " $(column_lcm_valid[i]), " * L"sigma = " * "$(column_sigmas_valid[i])", lw = 2, color = colors[i], markershape = :circle, tickfontsize=12, labelfontsize=15, legendfontsize=15, framestyle =:box, gridlinewidth=1, xminorticks=10, yminorticks=10)
