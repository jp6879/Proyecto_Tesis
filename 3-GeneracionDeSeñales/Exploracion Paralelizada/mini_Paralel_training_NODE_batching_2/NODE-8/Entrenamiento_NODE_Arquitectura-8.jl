...
θ = CSV.read("C:\\Users\\Propietario\\Desktop\\ib\\Tesis_V1\\Proyecto_Tesis\\3-GeneracionDeSeñales\\Exploracion Paralelizada\\mini_Paralel_training_NODE_batching_2\\Parameters\\8_Parameters.csv", DataFrame)
...
plot!(ttrain, Predict_Singals(U0[1], Signals_test_derivadas_train[:,1], ttrain), color = :red, label = "Entrenamiento", markershape = :star6, ls = :dash, lw = 2)
...
plot!(vcat(t_short,t_long), Signals_test[end,:], label = "Señal 100", xlabel = L"t", ylabel = L"S(t)", title = "Predicción de señales", lw = 2, color = :blue, markershape = :circle, tickfontsize=12, labelfontsize=15, legendfontsize=5, framestyle =:box, gridlinewidth=1, xminorticks=10, yminorticks=10)
plot!(ttrain, Predict_Singals(U0[end], Signals_test_derivadas_train[:,end], ttrain), color = :red, label = "Entrenamiento", markershape = :star6, ls = :dash, lw = 2)
...
for i in 2:size(Signals_test)[1]
    ...
    # plot!(ttrain, Predict_Singals(U0[i], Signals_test_derivadas_train[:,i], ttrain), color = :red, label = "Entrenamiento", markershape = :star6, ls = :dash, lw = 2)
